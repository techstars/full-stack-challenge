import { getRequest, patchRequest, postRequest, deleteRequest } from "util/api";
...
export const DELETE_COMPANY_DETAILS = 'DELETE_COMPANY_DETAILS';
...
export const deleteCompany = id => async dispatch => {
  dispatch({ type: LOAD_COMPANY_DETAILS });
  try {
    const response = await deleteRequest(`companies/${id}`);
    dispatch({ type: DELETE_COMPANY_DETAILS, payload: response });
    return response;
  } catch (error) {
    dispatch({ type: ERROR_COMPANY_DETAILS, payload: error });
    return { error }
