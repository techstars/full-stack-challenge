...
const {
  safeLoad
} = require('./node_modules/react-on-rails/node_modules/react-on-rails/node_modules/js-yaml');
...
const configLoader = configPath => {
  ...
  const configuration = safeLoad(readFileSync(ymlConfigPath, 'utf8'))[
    configEnv
  ];

  const devBuild = env !== 'production';
  const hotReloadingHost =
    configuration.hot_reloading_host || DEFAULT_HOT_RELOADING_HOST;

  // NOTE: Rails path is hard coded to `/public`
  const webpackPublicOutputDir =
    configuration.webpack_public_output_dir ||
    DEFAULT_WEBPACK_PUBLIC_OUTPUT_DIR;
  const webpackOutputPath = resolve(
    configPath,
    '..',
    'public',
    webpackPublicOutputDir
  ...
  const hotReloadingEnabled =
    env.HOT_RELOADING === 'TRUE' ||
    env.HOT_RELOADING === 'YES' ||
    configuration.hot_reloading_enabled_by_default ||
    HOT_RELOADING_ENABLED_BY_DEFAULT;
